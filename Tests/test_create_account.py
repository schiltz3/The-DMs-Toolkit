...
class TestCreateAccount(TestCase):
    ...
    def setUp(self):
        ...
        self.confirm_account_creation_url = "/create_account/confirm_account_creation/dummy"
        self.username = "dummy"
    ...
    def test_form_valid(self):
        ...
        form = CreateAccountForm(data={"email": self.email, "username": self.username, "password": self.password})
        self.assertTrue(form.is_valid())
        
    def test_form_invalid_email(self):
        """Tests if the form is valid after giving incorrect credentials."""
        form = CreateAccountForm(data={"email": "dummy", "username": self.username, "password": self.password})
        self.assertFalse(form.is_valid())
        
    def test_form_invalid_no_email(self):
        """Tests if the form is valid after giving incorrect credentials."""
        form = CreateAccountForm(data={"email": "", "username": self.username, "password": self.password})
        self.assertFalse(form.is_valid())
        
    def test_form_invalid_no_username(self):
        """Tests if the form is valid after giving incorrect credentials."""
        form = CreateAccountForm(data={"email": self.email, "username": "", "password": self.password})
        self.assertFalse(form.is_valid())
        
    def test_form_valid_no_password(self):
        """Tests if the form is valid after giving incorrect credentials."""
        form = CreateAccountForm(data={"email": self.email, "username": self.username, "password": ""})
        self.assertFalse(form.is_valid())
    ...
    def test_successful_account_creation(self):
        ...
        response = self.client.post(self.create_account_url, data={"email": self.email, "username": self.username, "password": self.password}, follow=True,)
        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, self.confirm_account_creation_url, status_code=301)
    ...
    def test_unsuccessful_account_creation_email(self):
        ...
        response = self.client.post(self.create_account_url, data={"email": "dummy", "username": self.username, "password": self.password}, follow=True,)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.redirect_chain, []) 
        
    def test_unsuccessful_account_creation_no_email(self):
        """Tests if a user is able to successfully create a new account giving incorrect credentials"""
        response = self.client.post(self.create_account_url, data={"email": "", "username": self.username, "password": self.password}, follow=True,)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.redirect_chain, []) 
    
    def test_unsuccessful_account_creation_no_username(self):
        """Tests if a user is able to successfully create a new account giving incorrect credentials"""
        response = self.client.post(self.create_account_url, data={"email": self.email, "username": "", "password": self.password}, follow=True,)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.redirect_chain, []) 
        
    def test_unsuccessful_account_creation_no_password(self):
        """Tests if a user is able to successfully create a new account giving incorrect credentials"""
        response = self.client.post(self.create_account_url, data={"email": self.email, "username": self.username, "password": ""}, follow=True,)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.redirect_chain, []) 
