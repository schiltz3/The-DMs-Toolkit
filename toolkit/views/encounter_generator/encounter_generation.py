...
class Encounter_Generator:
    ENCOUNTER_TYPE_LIST = ["Horde", "Scrimish", "Average Encounter" ,"Challenge", "Minor Boss", "Major Boss"]
    ...
    def calculate_average_cr(self):
        ...
        self.average_cr = sum/self.monster_list.__len__
    
    def generate_loot(self):
        self.generator = Loot_Generator()
        self.loot = self.generator.generate_loot(self, self.generator_key, int(self.average_party_level))
        self.dropped_loot = self.loot
    
    def generate_monster(self):
        if self.encounter_type == "Average Encounter":
            monster_possibilities = Monster.objects.filter(Challenge_Rating <= self.average_party_level+1)
            monster_possibilities.filter(Challenge_Rating >= ((self.average_party_level + (self.average_party_level-1))/2))
        elif self.encounter_type == "Horde":
            monster_possibilities = Monster.objects.filter(Challenge_Rating <= ((self.average_party_level + (self.average_party_level-2))/3))
            monster_possibilities.filter(Challenge_Rating >= ((self.average_party_level + (self.average_party_level-2))/4))
        elif self.encounter_type =="Scrimish":
            monster_possibilities = Monster.objects.filter(Challenge_Rating <= self.average_party_level)
            monster_possibilities.filter(Challenge_Rating >= ((self.average_party_level + (self.average_party_level-2))/2))
        elif self.encounter_type == "Challenge":
            if 10 > self.average_party_level > 1:
                monster_possibilities = Monster.objects.filter(Challenge_Rating <= ((self.average_party_level + (self.average_party_level+3))/2))
            elif self.average_party_level >= 10:
                monster_possibilities = Monster.objects.filter(Challenge_Rating <= (self.average_party_level+(self.average_party_level/3)))
            else:
                monster_possibilities = Monster.objects.filter(Challenge_Rating <= self.average_party_level)
            monster_possibilities.filter(Challenge_Rating >= self.average_party_level-1)
            
        elif self.encounter_type == "Minor Boss":
            if 10 > self.average_party_level > 3:
                monster_possibilities = Monster.objects.filter(Challenge_Rating <= ((self.average_party_level + (self.average_party_level+3))/2))
            elif self.average_party_level >= 10:
                monster_possibilities = Monster.objects.filter(Challenge_Rating <= (self.average_party_level+(self.average_party_level/3)+1))
            else:
                monster_possibilities = Monster.objects.filter(Challenge_Rating <= self.average_party_level+2)
    ...
    def generate_encounter(self, average_level = 1, encounter_type = "Average Encounter", tags = [], generator_key = "Random", loot_generate = False):
        monster_count = 0
